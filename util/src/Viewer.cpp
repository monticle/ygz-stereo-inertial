...
namespace ygz {
	...
	Viewer::Viewer(bool startViewer) {
		...
		//mLabelTrackFeats.push_back("Track Feats = ");
		mBiasLog.SetLabels(mBiasLogLabels);
		//mBiasLog.SetLabels(mLabelTrackFeats);
	}
	...
	void Viewer::Run() {
		...
		pangolin::CreateWindowAndBind("LeadSense Visual Odometry", 1280, 720);
		...
		pangolin::View &Visualization3D_display = pangolin::CreateDisplay() //mym
		...
		pangolin::CreateDisplay()
			.SetBounds(0.0, pangolin::Attach::Pix(UI_WIDTH* h / (float)w), 0.0, pangolin::Attach::Pix(UI_WIDTH))
			.AddDisplay(d_video);
		...
		pangolin::Plotter plotter_biascurve(&mBiasLog, 0.0f, 20.0f * 20, -5.0f, 5.0f, 0.1f, 0.002f);//mym
		plotter_biascurve.Track("$i");
		// plotter_biascurve.SetAspect(w / (float) h);

		/*     pangolin::CreateDisplay()
		.SetBounds(0.0, pangolin::Attach::Pix(UI_WIDTH* h / (float)w), pangolin::Attach::Pix(UI_WIDTH * 2), 1.0)
		.AddDisplay(plotter_biascurve); */

		pangolin::CreateDisplay()
			.SetBounds(0.0, pangolin::Attach::Pix(UI_WIDTH* h / (float)w), pangolin::Attach::Pix(UI_WIDTH * 1.0), 1.0)
			.AddDisplay(plotter_biascurve);
#endif

		//pangolin::CreatePanel("ui").SetBounds(0.6, 1.0, 0.0, pangolin::Attach::Pix(UI_WIDTH));
		pangolin::View& panelUI = pangolin::CreatePanel("ui");
		//panelUI.SetBounds(0.0, pangolin::Attach::Pix(UI_WIDTH* h / (float)w), pangolin::Attach::Pix(UI_WIDTH), pangolin::Attach::Pix(UI_WIDTH*2));//mym
		panelUI.SetBounds(0.0, pangolin::Attach::Pix(UI_WIDTH* h / (float)w), pangolin::Attach::Pix(UI_WIDTH), pangolin::Attach::Pix(UI_WIDTH * 1.0));//mym

#ifdef IS_SHOW_DATA
		pangolin::Var<int> nTrackFeats("ui.Track Inliers", 0); //mym
		pangolin::Var<int> nTrackState("ui.Track State", 0); //mym
#endif
		...
		while (!pangolin::ShouldQuit() && mbRunning) {
			...
			if (mCurrentFrame) {
				...
				if (mCurrentFrame->mTimeStamp != preFrameTime) {
					...
					Vector6d bias;
					int imuSize = mCurrentFrame->mvIMUDataSinceLastFrame.size();
					for (int i = 0; i < imuSize && i <10; i++) {
#ifdef IS_SHOW_DATA
						plotter_biascurve.Activate();

						bias.head<3>() = mpNewCurrent->mvIMUDataSinceLastFrame[i].mfGyro;
						bias.tail<3>() = mpNewCurrent->mvIMUDataSinceLastFrame[i].mfAcce;
			...
			// show status and track points
			{
				unique_lock<mutex> lk(mMutexTrackerStatus);
#ifdef IS_SHOW_DATA
				// nTrackFeats = mTrackInliers;//mym
				// nTrackState = mTrackState;//mym
#endif
			}
	...
	void Viewer::DrawTrajectory() {
		...
		glColor3d(255.0 / 255.0, 255.0 / 255.0, 0.0 / 255.0);//mym
